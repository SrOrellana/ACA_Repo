...
class DialogDeleteAdvisor: DialogFragment() {

    private var mBinding: DialogDeleteAdvisorBinding? = null
    ...
    override fun onCreateView(
        ...
        mBinding = DialogDeleteAdvisorBinding.inflate(inflater, container, false)
    ...
    private fun setNavigationListeners() {
        ...
        advisorViewModel.userData.observe(viewLifecycleOwner) { user ->
            binding.dialogConfirmRegisterActionOk.setOnClickListener {
                advisorViewModel.deleteAdvisor(user[0].accessToken, args.idAdvisor)
            }
        }
    }
    ...
    private fun handleUIState(uiState: UIState<Int>?) {
        when(uiState) {
            ...
            is UIState.Success -> {
                endShowProgressBar()
                val direction = DialogDeleteAdvisorDirections
                    .actionDialogDeleteAdvisorToAdminAdvisoryFragment()
