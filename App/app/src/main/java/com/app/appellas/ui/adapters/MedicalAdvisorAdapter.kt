...
class MedicalAdvisorAdapter:
    RecyclerView.Adapter<MedicalAdvisorAdapter.MedicalAdvisorViewHolder>() {

    private var medicalList: List<CreateAdvisorResponse>? = null

    private var rol: String = ""

    inner class MedicalAdvisorViewHolder(itemView: View): RecyclerView.ViewHolder(itemView)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MedicalAdvisorViewHolder {
        val card = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_advisory, parent, false)
        return MedicalAdvisorViewHolder(card)
    }

    override fun onBindViewHolder(holder: MedicalAdvisorViewHolder, position: Int) {
        val medical = medicalList?.get(position)
    ...
    override fun getItemCount(): Int = medicalList?.size ?: 0
